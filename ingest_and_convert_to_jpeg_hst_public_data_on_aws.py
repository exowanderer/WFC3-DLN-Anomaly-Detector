...
if all_saves_exist: # Load them
    ...
    # filtered_dict = {ir_filt:[Observations.filter_products(product_tbl, 
    #                                         mrp_only=False, productSubGroupDescription='FLT', 
    #                                         dataproduct_type='image')
    #                                         for product_tbl in product_dict_full[ir_filt]]
    #                                         for ir_filt in tqdm(WFC3IR_Filters)}
    ...
    for ir_filt in tqdm(WFC3IR_Filters):
        ...
        for kf, filtered_tbl in tqdm(enumerate(filtered_dict[ir_filt]), total=len(filtered_dict[ir_filt])):
            try:
                ...
                print(kf, ir_filt, str(e))
    
    joblib.dump(s3_urls_dict, 's3_urls_dict_list_WFC3IR_Filiters.joblib.save')

# Just download a few of the files (remove the [:nUrls] to download them all)
# for ir_filt, s3_urls_now in s3_urls_dict.items():
#     for url in s3_urls_now[:nUrls]:
...
nUrls = 100
for ir_filt, s3_urls_now in tqdm(s3_urls_dict.items(), total=len(s3_urls_dict)):
    for url in tqdm(s3_urls_now, total=len(s3_urls_now)):#[:nUrls]
        ...
        if root.replace('.fits','.jpg') not in existing_JPEGs:
            bucket.download_file(fits_s3_key, fits_filename, ExtraArgs={"RequestPayer": "requester"})
